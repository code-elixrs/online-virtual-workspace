...
const RoomControls = ({ 
  ...
  const handleToggleMic = async () => {
    if (onToggleAudio) {
      const enabled = await onToggleAudio() // Made async to handle permission requests
  ...
  const handleToggleVideo = async () => {
    if (onToggleVideo) {
      const enabled = await onToggleVideo() // Made async to handle permission requests
      console.log(`📹 Video ${enabled ? 'enabled' : 'disabled'}`)
    }
  }

  // Generate status message based on WebRTC state
  const getStatusMessage = () => {
    if (webrtcError) {
      return { text: `Error: ${webrtcError}`, color: 'text-red-400' }
    }
    ...
    if (!hasRequestedMedia) {
      return { text: 'Click video/audio to join', color: 'text-blue-400' }
    }
    
    if (connectionSummary) {
      const { connectedPeers, connectingPeers, remoteStreams, totalConnections } = connectionSummary
      
      if (totalConnections === 0) {
        return { text: 'Ready - No other users', color: 'text-green-400' }
      }
      
      if (connectingPeers > 0) {
        return { text: `Connecting to ${connectingPeers} user${connectingPeers > 1 ? 's' : ''}...`, color: 'text-blue-400' }
      }
      
      if (connectedPeers > 0) {
        return { 
          text: `Connected to ${connectedPeers} user${connectedPeers > 1 ? 's' : ''} • ${remoteStreams} stream${remoteStreams !== 1 ? 's' : ''}`, 
    ...
    <div className="bg-gray-800 border-b-4 border-blue-600 p-4 shadow-lg">
      <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            ...
            <div className={`w-3 h-3 rounded-full ${
              webrtcError ? 'bg-red-500' : 
              !isWebRTCInitialized || isConnecting ? 'bg-yellow-500 animate-pulse' : 
              'bg-green-500'
            }`}></div>
            <div>
              <h2 className="text-xl font-bold text-white">{roomName}</h2>
              <p className="text-sm text-gray-400">
                Welcome, {user.name} • <span className={status.color}>{status.text}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          {/* Microphone - Enhanced with loading state */}
          <button
            onClick={handleToggleMic}
            disabled={!isWebRTCInitialized || isConnecting}
            className={`p-3 rounded-full transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${
              isConnecting ? 'bg-blue-600 animate-pulse' :
              hasRequestedMedia 
                ? (isAudioEnabled 
                    ? 'bg-green-600 hover:bg-green-700 text-white shadow-lg' 
                    : 'bg-red-600 hover:bg-red-700 text-white shadow-lg')
                : 'bg-blue-600 hover:bg-blue-700 text-white shadow-lg ring-2 ring-blue-300'
            }`}
            title={
          ...
          >
            {isConnecting ? (
              ...
              // Show microphone with plus icon when permissions not requested
          ...
          {/* Video - Enhanced with loading state */}
          <button
          ...
          >
            {isConnecting ? (
              ...
              // Show video with plus icon when permissions not requested
              <div className="relative">

    ...
    <div className="bg-gray-800 border-b-4 border-blue-600 p-4 shadow-lg">
      <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
       ...
       <div className="flex items-center space-x-4">
         <div className="flex items-center space-x-2">
           <div className={`w-3 h-3 rounded-full animate-pulse ${
             isWebRTCInitialized ? 'bg-green-500' : 'bg-yellow-500'
           }`}></div>
           <div>
             <h2 className="text-xl font-bold text-white">{roomName}</h2>
             <p className="text-sm text-gray-400">
               Welcome, {user.name}
               {webrtcError && <span className="text-red-400 ml-2">• {webrtcError}</span>}
               {!isWebRTCInitialized && !webrtcError && <span className="text-yellow-400 ml-2">• Connecting...</span>}
               {!hasRequestedMedia && <span className="text-blue-400 ml-2">• Click video/audio to join</span>}
             </p>
           </div>
         </div>
       </div>
       ...
       <div className="flex items-center space-x-3">
         {/* Microphone - Show permission request state */}
         <button
           ...
           className={`p-3 rounded-full transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${
             hasRequestedMedia 
               ...
               : 'bg-blue-600 hover:bg-blue-700 text-white shadow-lg ring-2 ring-blue-300 animate-pulse'
           }`}
           title={
             !hasRequestedMedia 
         ...
         {/* Video - Show permission request state */}
         <button
